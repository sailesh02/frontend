...
} from "./preApprovedPlanResource/scrutinyDetails";
import { applicantDetails } from "./preApprovedPlanResource/applicantDetails";
import { detailsofplot } from "./preApprovedPlanResource/boundarydetails";
import {
  documentDetails,
  additionalDocsInformation,
} from "./preApprovedPlanResource/documentDetails";
...
import { bpaSummaryDetails } from "./preApproveSummaryDetails";
...
const selectLicenceType = (state, dispatch) => {
  ...
  /*if(isTrue) {
      ...
      dispatch(
        handleField("preApprovedPlanApply", "components.cityPickerDialog", "props.open", !toggle)
      );
...
const setMohallaIfNotSet = async (state, dispatch, action, cityValue) => {
  try {
    ...
    let boundries =
      ...
      payload.TenantBoundary[0].boundary.filter(
