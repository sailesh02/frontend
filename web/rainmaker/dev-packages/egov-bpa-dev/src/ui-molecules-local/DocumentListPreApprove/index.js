...
const acceptedFiles = acceptedExt => {
...
const isFileValid = (file, acceptedFiles) => {
  let mimeType = file["type"];
  if(mimeType==="") {
    const fileNameArray = file["name"].split(".");
    mimeType = fileNameArray[fileNameArray.length - 1];
    return (
      (mimeType &&
        acceptedFiles &&
        acceptedFiles.indexOf(mimeType) > -1) ||
      false
    );
  }
  return (
...
const handleFileUpload = (event, handleDocument, props) => {
  const S3_BUCKET = {
...
class DocumentListPreApprove extends Component {
  state = {
    uploadedDocIndex: 0
  };
...
DocumentListPreApprove.propTypes = {
