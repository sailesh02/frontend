...
export const callPGService = async (state, dispatch) => {
  ...
  }/mr-citizen/PaymentRedirectPage`;
  try {
    const queryObj = [
      {
        key: "tenantId",
...
const moveToSuccess = (href, dispatch, receiptNumber) => {
  ...
  dispatch(
    setRoute(
      `/mr/acknowledgement?purpose=${purpose}&status=${status}&applicationNumber=${applicationNo}&tenantId=${tenantId}&secondNumber=${receiptNumber}`
    )
  );
};
